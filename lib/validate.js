...
module.exports = async function validate(ticket, context) {
    ...
    parseXML(xml, {
        ...
        if (opts.casInfoFormat) {
            ctx.session[opts.session.key] = opts.casInfoFormat(serviceResponse.authenticationSuccess);
        } else {
            ctx.session[opts.session.key] = serviceResponse.authenticationSuccess;
        }
    });
}